...
exports.add = async (req, res) => {
    ...
    try {
        Parallel.findOne({ date: newParallel.date }, function (err, docs) {
            // if (docs.length == 0) {
                //save 
                newParallel.save(function (err) {
                    if (err) {
                        console.log(err);
                        return err;
                    }
                    console.log("Parallel Session created");

                    return res.status(200).send(newParallel);
                })
            // } else {
...
exports.get = async (req, res) => {
    ...
    try {
        const lectufsarer2 = await Parallel.find();
